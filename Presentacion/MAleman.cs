...
{
    public partial class MAleman : Form
    {
        public MAleman()
        {
            InitializeComponent();
        ...
        {
            double PERIODOS = 0;
            switch (cboPer_Tasa_Ale.SelectedIndex)
            {
        ...
        private double Calculo_TEA(double i)
        {
            ...
            switch (cboCap_Ale.SelectedIndex)
            {
        ...
        private void Calcular_Amortizacion()
        {
            ...
            // DATOS DE ENTRADA
            double PRECIOVENTA = Convert.ToDouble(nudPrecioVA.Value);
            double CUOTAINICIAL = Convert.ToDouble(nudCuotaA.Value) / 100;
            double PRESTAMO = PRECIOVENTA - (PRECIOVENTA * CUOTAINICIAL);
            double TEA = Calculo_TEA(Convert.ToDouble(nudTEAA.Value)) / 100; ;
            ...
            double TEP = Calculo_Tasas_Periodos(Convert.ToDouble(nudTEAA.Value)) / 100;
            //double PAGUITO = (PRESTAMO * TEP) / (1 - Math.Pow(1 + TEP, -PERIODOS));
            double TASADESC = Convert.ToDouble(this.nudescuento.Value);


            // RESULTADOS
            int DIAS = Convert.ToInt32(txtNumPerTasa.Text);
            DateTime FECHA_PROGRAMADA = FECHAEMISION;
            double INTERES, AMORTIZACION, CUOTA, SALDOINICIAL, SALDOFINAL = PRESTAMO;
            ...
            if (PRECIOVENTA == 0 || cbomonentra.Text == "" || cboTipo_Tasa.Text == "" || TEA == 0 || cboPer_Tasa_Ale.Text == "" || ANIOS == 0)
            {
            ...
            else
            {
                for (int i = 0; i < PERIODOS + 1; i++)
                {
                    ...
                    {
                        dgvMetal.Rows.Add(i, FECHA_PROGRAMADA.ToShortDateString(), Math.Round(TEA * 100, 7) + " %", Math.Round(TEP * 100, 7) + " %", "s",
        ...
        public void ExportarExcel(DataGridView dataaleman)
        {
            ...
            int indicecolumna = 0;
            foreach (DataGridViewColumn columna in dataaleman.Columns)
            {
                indicecolumna++;
                exporta.Cells[1, indicecolumna] = columna.Name;
            }
            int indicefila = 0;
            foreach (DataGridViewRow fila in dataaleman.Rows)
            {
                indicefila++;
                indicecolumna = 0;
                foreach (DataGridViewColumn columna in dataaleman.Columns)
        ...
        private void cboTipo_Tasa_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cboTipo_Tasa.Text == "Efectiva")
            {
                ...
                cboCap_Ale.Visible = false;
                txtNumCap.Visible = false;
            }
            if (cboTipo_Tasa.Text == "Nominal")
            {
                lblFrec_Cap.Visible = true;
                cboCap_Ale.Visible = true;
                txtNumCap.Visible = true;
            }
        }
        private void cboPer_Tasa_Ale_SelectedIndexChanged(object sender, EventArgs e)
        {
            switch (cboPer_Tasa_Ale.SelectedIndex)
            {
            ...
            lblCokPeriodo.Text = "COK " + cboPer_Tasa_Ale.Text + " (%):";
        }
        private void cboCap_Ale_SelectedIndexChanged(object sender, EventArgs e)
        {
            switch (cboCap_Ale.SelectedIndex)
            {
