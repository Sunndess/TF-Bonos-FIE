...
{
    public partial class MFrances : Form
    {
        ...
        {
            double PERIODOS = 0;
            switch (cboPer_Tasa_Fran.SelectedIndex)
            {
        ...
        private double Calculo_TEA(double i)
        {
            ...
            switch (cboCap_Fran.SelectedIndex)
            {
        ...
        private void Calcular_Amortizacion()
        {
            ...
            double CUOTA = (PRESTAMO * TEP) / (1 - Math.Pow(1 + TEP, -PERIODOS));
            ...
            double INTERES, AMORTIZACION, SALDOINICIAL, SALDOFINAL = PRESTAMO;
            ...
            if (PRECIOVENTA == 0 || cbomonentra.Text == "" || cboTipo_Tasa.Text == "" || TEA == 0 || cboPer_Tasa_Fran.Text == "" || ANIOS == 0)
            {
            ...
            else
            {
                for (int i = 1; i < PERIODOS + 1; i++)
                {
                    ...
                    {
                        dgvMF.Rows.Add(i, FECHA_PROGRAMADA.ToShortDateString(), Math.Round(TEA * 100, 7) + " %", Math.Round(TEP * 100, 7) + " %", "s",
        ...
        public void ExportarExcel(DataGridView datafrances)
        {
            ...
            int indicecolumna = 0;
            foreach (DataGridViewColumn columna in datafrances.Columns)
            {
                indicecolumna++;
                exporta.Cells[1, indicecolumna] = columna.Name;
            }
            int indicefila = 0;
            foreach (DataGridViewRow fila in datafrances.Rows)
            {
                indicefila++;
                indicecolumna = 0;
                foreach (DataGridViewColumn columna in datafrances.Columns)
        ...
        private void cboTipo_Tasa_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cboTipo_Tasa.Text == "Efectiva")
            {
                ...
                cboCap_Fran.Visible = false;
                txtNumCap.Visible = false;
            }
            if (cboTipo_Tasa.Text == "Nominal")
            {
                lblFrec_Cap.Visible = true;
                cboCap_Fran.Visible = true;
                txtNumCap.Visible = true;
            }
        }

        private void cboPer_Tasa_Fran_SelectedIndexChanged(object sender, EventArgs e)
        {
            switch (cboPer_Tasa_Fran.SelectedIndex)
            {
            ...
            lblCokPeriodo.Text = "COK " + cboPer_Tasa_Fran.Text + " (%):";
        }

        private void cboCap_Fran_SelectedIndexChanged(object sender, EventArgs e)
        {
            switch (cboCap_Fran.SelectedIndex)
            {
